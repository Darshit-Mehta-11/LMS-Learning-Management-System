...
export async function buyCourse(token, coursesId, userDetails, navigate, dispatch) {
    ...
    try {
        const RAZORPAY_KEY = import.meta.env.VITE_APP_RAZORPAY_KEY || "dummy";
        ...
        // MOCK BEHAVIOR: if Razorpay key is dummy, bypass Razorpay dialog completely
        if (RAZORPAY_KEY === "dummy") {
            const mockResponse = {
                razorpay_order_id: orderResponse.data.message.id,
                razorpay_payment_id: "pay_dummy_" + Date.now(),
                razorpay_signature: "dummy_signature"
            };
            // Directly verify the dummy payment which triggers enrollment on backend
