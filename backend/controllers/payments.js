...
exports.capturePayment = async (req, res) => {
    ...
    try {
        if (process.env.RAZORPAY_KEY === 'dummy') {
            return res.status(200).json({
                success: true,
                message: {
                    id: 'order_dummy_' + Date.now().toString(),
...
exports.verifyPayment = async (req, res) => {
    ...
    // Bypass signature check for dummy local dev transactions
    if (razorpay_signature === 'dummy_signature' && process.env.RAZORPAY_KEY === 'dummy') {
        try {
            //enroll student
