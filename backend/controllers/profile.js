...
exports.updateUserProfileImage = async (req, res) => {
    try {
        ...
        // validation
        console.log('DEBUG: updateUserProfileImage - req.files:', req.files);
        if (!profileImage) {
            console.log('DEBUG: updateUserProfileImage - No profileImage in req.files');
        ...
        console.log('DEBUG: updateUserProfileImage - profileImage tempFilePath:', profileImage.tempFilePath);

        // Ensure unique filename
        const filename = `${Date.now()}-${profileImage.name}`;
        const uploadPath = require('path').join(__dirname, '..', 'public', 'images', filename);

        // Move the file locally
        await profileImage.mv(uploadPath);

        const imageUrl = `${process.env.VITE_APP_BASE_URL || 'http://localhost:5000'}/images/${filename}`;
        console.log('DEBUG: updateUserProfileImage - Local image saved at:', imageUrl);
